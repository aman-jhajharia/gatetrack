...
import { Loader2, Users, BookOpen, BookMarked, ClipboardList } from 'lucide-react';
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div>
        ...
        <p className="text-slate-500 text-sm">Platform overview and student management</p>
      </div>

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatsCard title="Total Students" value={stats?.totalStudents ?? 0} icon={Users} iconColor="text-indigo-400" />
        <StatsCard title="Subjects" value={stats?.totalSubjects ?? 0} icon={BookOpen} iconColor="text-emerald-400" />
        <StatsCard title="Lectures" value={stats?.totalLectures ?? 0} icon={ClipboardList} iconColor="text-amber-400" />
        <StatsCard title="Vocab Words" value={stats?.totalVocab ?? 0} icon={BookMarked} iconColor="text-purple-400" />
      </div>

      <div className="glass-card rounded-xl overflow-hidden">
        <div className="p-4 border-b border-white/5">
          <h2 className="text-sm font-semibold text-white">Registered Students</h2>
        </div>
        {!stats?.students?.length ? (
          <div className="p-8 text-center text-slate-500 text-sm">No students yet. Create some from the Users page.</div>
        ) : (
